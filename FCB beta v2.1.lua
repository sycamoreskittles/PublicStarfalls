...
-- Recoded the offset to be easily changable, you can now change it with the "offset" variable! (must be a vector)
-- Done various code optimizations, such as restricting some unused/debug code
-- Added command for locking the position of the chip (?sf.FTB.lockpos) // FIX: make it work
-- Added command for setting the destination of the chip to be the owner's center of mass // DO: make this
